...
file_path = 'D:/pythonProject/parameters_bigsample100km_29.xlsx'
...
df_D_ij = pd.read_excel(file_path, sheet_name='D_ij', usecols='B:AAW', skiprows=0, nrows=2263)   # 距离矩阵
D_ij = df_D_ij.values
...
mdl.addConstr(
    gp.quicksum(a[i] for i in range(S)) >= 11962067248,
    name="reduction_target_sources"
)
print(8)

mdl.addConstr(
    gp.quicksum(b[j] for j in range(R)) >= 11962067248,
