...
file_path = 'D:/pythonProject/parameters_bigsample100km_39.xlsx'
...
df_D_ij = pd.read_excel(file_path, sheet_name='D_ij', usecols='B:AAO', skiprows=0, nrows=1789)   # 距离矩阵
D_ij = df_D_ij.values
...
mdl.addConstr(
    gp.quicksum(a[i] for i in range(S)) >= 15475500000,
    name="reduction_target_sources"
)
print(8)

mdl.addConstr(
    gp.quicksum(b[j] for j in range(R)) >= 15475500000,
...
objective = (
    ...
    gp.quicksum(0.01 * 1.1 * D_ij[i, j] * x[i, j] / scaling_factor for i in range(S) for j in range(R)) +
...
max_runtime = 5000  # 设置最大运行时间为5000S
...
try:
    ...
    if mdl.solCount > 0:
        ...
        df_a.to_excel('D:/pythonProject/a1_values.xlsx', index=False)
        print(14)

        # 导出 b[j]
        b_values = [b[j].x for j in range(R)]
        df_b = pd.DataFrame({'b[j]': b_values})
        df_b.to_excel('D:/pythonProject/b1_values.xlsx', index=False)
        print(15)

        # 导出 x[i][j]
        x_values = [[x[i, j].x for j in range(R)] for i in range(S)]
        df_x = pd.DataFrame(x_values)
        df_x.to_excel('D:/pythonProject/x1_values.xlsx', index=False, header=[f'x[{j}]' for j in range(R)])
        print(16)
        ...
        for start_row in range(0, len(y_flat), max_rows_per_file):
            ...
            file_name = f'D:/pythonProject/y1_values_batch_{start_row // max_rows_per_file + 1}.xlsx'
